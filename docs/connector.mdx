...
### Sync additional Salesforce user fields

The connector syncs a fixed set of standard fields from the Salesforce User object. If your org keeps access-relevant information in another field — a custom picklist such as `Role_Based_Access__c`, for example — list that field in the **Additional User Fields** setting and the connector pulls its value into the user's profile in C1, where you can map it onto a user attribute for access reviews.

Use the exact Salesforce field API name, including the `__c` suffix on custom fields (for example, `Role_Based_Access__c`, `Department`, `FederationIdentifier`). This is read-only: the connector never writes these fields back to Salesforce.

<Note>
Relationship traversals (such as `Manager.Name`) are not supported — name fields that live directly on the User object.

You can list up to 50 fields; extras are dropped with a warning in the logs.

A field name that doesn't exist on the User object, or that the connector's Salesforce user can't see, is skipped with a warning in the logs. The rest of the sync is unaffected, so a typo here can't stop your users from syncing. After you correct a field name, or grant the connector's Salesforce user access to a field it couldn't see, the change is picked up automatically: within an hour of the previous sync, because the connector caches the User object's field list, or simply on the next sync if you sync less frequently than that.

Numeric fields are synced as numbers, and values above 9,007,199,254,740,992 (2^53) lose precision — an 18-digit Number or Auto Number field arrives rounded. If you need an exact large identifier, use a Text or Formula (Text) field on the User object instead.
</Note>
...
<Step>
   If you chose **OAuth**: 
      ...
      7. **Optional.** In the **Additional User Fields** setting, list the API names of any extra Salesforce User fields to sync into user profiles, such as `Role_Based_Access__c`.

      8. Click **Save**. 

      9. Click **Login with OAuth**.

      10. Log in and authorize C1 with your Salesforce instance.

      11. You will then be redirected back to the Salesforce setup page in C1, where you'll see an authorization message.
   ...
   If you chose **JWT Bearer**:
      ...
      6. **Optional.** Configure sync options as needed (connected apps, deactivated users, non-standard users, additional user fields, license mapping).
   ...
   If you chose **Client Credentials**:
      ...
      4. **Optional.** Configure sync options as needed (connected apps, deactivated users, non-standard users, additional user fields, license mapping).
   ...
   If you chose **Username and password (deprecated)**:
      ...
      9. **Optional.** In the **Additional User Fields** setting, list the API names of any extra Salesforce User fields to sync into user profiles, such as `Role_Based_Access__c`.
...
stringData:
  ...
  # Optional: include to sync extra Salesforce User fields into user profiles (comma-separated field API names)
  BATON_SYNC_ADDITIONAL_USER_FIELDS: Role_Based_Access__c
