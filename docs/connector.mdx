...
A field name that doesn't exist on the User object, or that the connector's Salesforce user can't see, is skipped with a warning in the logs. The rest of the sync is unaffected, so a typo here can't stop your users from syncing.
