...
func roleResource(_ context.Context, role *client.SalesforceRole) (*v2.Resource, error) {
...
func (o *roleBuilder) Grant(
	...
	if principal.Id.ResourceType != resourceTypeUser.Id {
		logger.Warn(
			"salesforce-connector: only users can be granted roles",
			zap.String("principal_type", principal.Id.ResourceType),
			zap.String("principal_id", principal.Id.Resource),
		)
		return nil, fmt.Errorf("salesforce-connector: only users can be granted roles")
	}
